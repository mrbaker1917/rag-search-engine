...
def generate_answer(search_results, query, limit=5):
    ...
    prompt = f"""You are a RAG agent for Hoopla, a movie streaming service. Your task is to provide a natural-language answer to the user's query based on documents retrieved during search. Provide a comprehensive answer that addresses the user's query.
...
def generate_summary(search_results, query, limit):
    ...
    The goal is to provide comprehensive information so that users know what their options are. Your response should be 
    information-dense and concise, with several key pieces of information about the genre, plot, etc. of each movie. 
...
def generate_citations(search_results, query, limit):
    context = ""
    for result in search_results[:limit]:
        context += f"{result['title']}: {result['document']}\n\n"
...
def citations_command(query, limit=DEFAULT_SEARCH_LIMIT):
    ...
    answer = generate_citations(search_results, query, limit)
